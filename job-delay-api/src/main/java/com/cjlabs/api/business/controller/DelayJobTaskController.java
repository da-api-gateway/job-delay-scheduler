...
@Slf4j
@RestController
@RequestMapping("/api/delayJobTask")
public class DelayJobTaskController {
    ...
    public FmkResult<Boolean> update(@RequestBody FmkRequest<DelayJobTaskReqUpdate> input) {
        boolean result = delayJobTaskApiService.update(input);
        return FmkResult.success(result);
    }
    ...
    public FmkResult<Boolean> delete(@RequestBody FmkRequest<Void> input) {
        boolean result = delayJobTaskApiService.deleteById(input);
        return FmkResult.success(result);
    }
